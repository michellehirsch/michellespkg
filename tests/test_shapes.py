...
def test_plot_rectangle_invalid_input():
    with pytest.raises(ValueError, match="Length and width must be positive for visualization."):
        michellespkg.plot_rectangle(length=-10, width=5)
    with pytest.raises(ValueError, match="Length and width must be positive for visualization."):
        michellespkg.plot_rectangle(length=10, width=-5)
    with pytest.raises(ValueError, match="Length and width must be positive for visualization."):
...
def test_plot_triangle_invalid_input():
    with pytest.raises(ValueError, match="Base and height must be positive for visualization."):
        michellespkg.plot_triangle(base=-8, height=4)
    with pytest.raises(ValueError, match="Base and height must be positive for visualization."):
        michellespkg.plot_triangle(base=8, height=-4)
    with pytest.raises(ValueError, match="Base and height must be positive for visualization."):
